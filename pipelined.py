...
class model_bases_distance(object):
    ...
    def stock_prep(self,stock_X):
        ...
        train_stock_X_names_df = stock_X[[ENTITY, TARGET]]
        stock_X_norm = mms.fit_transform(stock_X[numeric_cols])
        train_stock_X_norm_df = pd.DataFrame(stock_X_norm, columns=numeric_cols_prep)
    ...
    def fit(self, stock_X):
        stock_X_prep = self.stock_prep(stock_X)
        x, y, p = prepare_stock_windows(stock_X_prep, 10,1, [1], [TARGET])
        self.model.fit(x,y)
...
def get_similarity(df_stocks, stock_to_compare, stock_names, similarity_func, experiment_path, force = False, split_time = ""):
    ...
    if (not os.path.isfile(similarities_path)) or force:
        if isinstance(similarity_func, model_bases_distance):
            ...
            similarities = [
                similarity_func.apply_distance(df_stocks[df_stocks[ENTITY] == stock_name][TARGET])
...
def main():
    ...
    experiment_params = {
        ...
        'k' : [1,  10, 50],
        'select_k_func' : [get_random_k, get_top_k ],#, get_top_k],
        'window_len' : [10],#, 5, 20],
        'slide' : [1],#, 3, 5, 10],
        'preprocessing_pipeline' : [Pipeline([('minmax_normalize', MinMaxScaler())])],
        'similarity_func' : [model_bases_distance(RandomForestClassifier(100)),apply_euclidean, apply_pearson,apply_dtw],
